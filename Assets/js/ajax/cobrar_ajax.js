function ObtenerCuentaCobrar(id) {
  fetch('index.php?url=cobrar&action=obtener&ID=' + id)
    ...
    .then(data => {
      document.getElementById('id').value = data.id_cuenta_cobrar;
...
function RegistrarPagoCuenta(id, saldoActual) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      form.method = 'POST';
      form.action = 'index.php?url=cobrar&action=registrar_pago';
