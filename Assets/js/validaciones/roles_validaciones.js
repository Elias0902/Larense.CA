...
function validar_nombre() {
  ...
  var error = document.getElementById('errorRol');
  ...
  if (valor.length < 5 || valor.length > 20) {
    error.innerHTML = 'El nombre del rol debe tener minimo 5 y maximo 20 caracteres.';
    nombre.classList.add('input-error');
    nombre.classList.add('is-invalid');  // clase bootstrap para borde rojo
    nombre.classList.remove('is-valid');
    return false;
  ...
  if (!regex.test(valor)) {
    error.innerHTML = "El nombre del rol no debe tener catacteres especiales y tampoco numeros  ej:Bebida .";
    nombre.classList.add('input-error');
    nombre.classList.add('is-invalid');  // clase bootstrap para borde rojo
    nombre.classList.remove('is-valid');
    return false;
...
function validar_nombre_modificado() {
  ...
  var nombre = document.getElementById('nombreRolEdit');
  var error = document.getElementById('errorRolEdit');
...
function EliminarRol(event, id){
  ...
  const url = "index.php?url=roles&action=eliminar&ID=" + id; 

  // confirmacion de la accion
  Swal.fire({
  title: '¿Estás seguro?',
  text: "¡Deseas eliminar el rol. No podrás revertir esto!",
  icon: 'warning',
