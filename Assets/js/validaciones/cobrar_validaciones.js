// Funciones de validación para Cuentas por Cobrar
...
function actualizarSaldoModificado() {
  ...
  // Solo actualizar si el saldo es 0 o igual al monto anterior
...
function validar_formulario() {
  ...
  if (cliente_valido && monto_valido && fecha_emision_valida && fechas_validas && descripcion_valida) {
    return true;
  } else {
    Swal.fire({
      icon: 'error',
...
function EliminarCuentaCobrar(event, id) {
  event.preventDefault();
  const url = "index.php?url=cobrar&action=eliminar&ID=" + id;

  Swal.fire({
    title: '¿Estás seguro?',
    text: "¡Deseas eliminar esta cuenta por cobrar?",
    icon: 'warning',
