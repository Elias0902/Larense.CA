...
class CuentaPagar extends Conexion {
    ...
    private function setPagarData($cuenta_json) {
        ...
        $proveedor_id = trim($cuenta['proveedor_id'] ?? '');
        ...
        // Validar compra_id (opcional)
        $compra_id = trim($cuenta['compra_id'] ?? '');
        if ($compra_id !== '' && (!preg_match($expre_id, $compra_id) || strlen($compra_id) > 10 || $compra_id < 0)) {
            return ['status' => false, 'msj' => 'El ID de la compra es invalido'];
        }
        $this->cuenta_compra_id = $compra_id ?: null;
    ...
    private function getCuentaProveedorID() { return $this->cuenta_proveedor_id; }
    private function getCuentaCompraID() { return $this->cuenta_compra_id; }
    ...
    private function Guardar_Cuenta() {
        ...
        try {
            $conn = $this->getConnectionNegocio();
            $query = "INSERT INTO cuentas_pagar (proveedor_id, compra_id, monto, saldo, fecha_emision, fecha_vencimiento, estado, descripcion)
                      VALUES (:proveedor_id, :compra_id, :monto, :saldo, :fecha_emision, :fecha_vencimiento, :estado, :descripcion)";
            $stmt = $conn->prepare($query);
            $stmt->bindValue(':proveedor_id', $this->getCuentaProveedorID());
            $stmt->bindValue(':compra_id', $this->getCuentaCompraID());
            ...
            if ($stmt->execute()) {
                return ['status' => true, 'msj' => 'Cuenta por pagar registrada con éxito.'];
            } else {
    ...
    private function Eliminar_Cuenta() {
        ...
        try {
            $conn = $this->getConnectionNegocio();
            $query = "UPDATE cuentas_pagar SET status = 0 WHERE id_cuenta_pagar = :id";
            $stmt = $conn->prepare($query);
            $stmt->bindValue(':id', $this->getCuentaID());
    ...
    private function Registrar_Pago($monto_pago) {
        ...
        try {
            ...
            $query_saldo = "SELECT saldo, monto FROM cuentas_pagar WHERE id_cuenta_pagar = :id";
    ...
    private function CambiarEstado_Cuenta($nuevo_estado) {
        ...
        try {
            $conn = $this->getConnectionNegocio();
            $query = "UPDATE cuentas_pagar SET estado = :estado WHERE id_cuenta_pagar = :id";
            $stmt = $conn->prepare($query);
            $stmt->bindValue(':id', $this->getCuentaID());
            $stmt->bindValue(':estado', $nuevo_estado);
